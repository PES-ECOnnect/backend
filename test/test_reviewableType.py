...
def test_createProductType():
    response = app.test_client().post("/products/types?token=93003eec-b589-11ec-a4e2-00155d3ce0fb&name=typeTest", data=json.dumps({'questions':['q1', 'q2']}), content_type='application/json')
    assert response.status_code == 200
    assert response.data == b'{"status":"success"}\n'

def test_cleanDB():
    id = rev.getReviewableTypeIdByName('typeTest')
    db.delete("DELETE FROM question where typeid = %s", (id,))
    db.delete("DELETE FROM reviewableType where name = 'typeTest'")
